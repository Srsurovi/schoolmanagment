...
<div class="col-md-11">
  ...
  <div class="card card-body">
	...
	<?php
		...
		$query='SELECT distinct class FROM  payment ';
		$query2='SELECT distinct section FROM  payment ';
		$query3='SELECT distinct month FROM  payment ';
	...
	<form action="showpayment.php" method="post">
	  ...
	  <div class="form-group">
		...
		<select name="month">
			<option value='Select Name'>Select Month</option>
			<?php
				while(list($month)=$result3->fetch_row()){
					echo "
						<option value='$month'>$month</option>";
					}
			?>
			<input type="hidden" value="<?php echo $row['month'] ?>" name="date" />
		</select>
	  </div>
